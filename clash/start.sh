...
if [ ! "`which curl`" ];then 
	echo "未安装curl"
	echo "Ubuntu/Debian可执行 sudo apt install curl"
	echo "CentOS可执行 sudo yum install curl"
	exit
fi
# 获取脚本工作目录绝对路径
...
# 尝试使用curl进行下载
curl  -A "clash-verge"-L -k -sS --retry 5 -m 10 -o $Temp_Dir/clash.yaml $URL
ReturnStatus=$?
...
PublicIP=`curl -s 4.ipw.cn`
# Output Dashboard access address and Secret
echo ''
echo -e "Clash面板地址"
echo -e "局域网请访问     http://$IP:9090/ui/?hostname=$IP&port=9090&secret=${Secret}&theme=auto"
echo -e "公网环境请访问   http://$PublicIP:9090/ui/?hostname=$PublicIP&port=9090&secret=${Secret}&theme=auto"
echo -e "本机运行请访问   http://127.0.0.1:9090/ui/?hostname=127.0.0.1&port=9090&secret=${Secret}&theme=auto"
echo -e 公网及局域网访问需注意防火墙放行""
...
echo -e "请手动执行命令开启系统代理  proxyon"
echo -e "临时关闭系统代理可执行:     proxyoff\n"
